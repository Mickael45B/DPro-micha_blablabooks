import db from "./db/connect_DB.js";
import Redis from 'ioredis';

// Singleton Redis lazy
let redisClient = null;
export const getRedis = () => {
  if (redisClient) return redisClient;
  if (process.env.DISABLE_REDIS || process.env.NODE_ENV === 'test') {
    console.log('[REDIS] disabled in this environment (DISABLE_REDIS=true or NODE_ENV=test)');
    return null;
  }
  redisClient = new Redis({
    host: process.env.REDIS_HOST || 'localhost',
    port: process.env.REDIS_PORT || 6379,
    password: process.env.REDIS_PASSWORD || undefined,
    retryStrategy: (times) => Math.min(times * 50, 2000),
    maxRetriesPerRequest: 3,
  });
  redisClient.on('error', (err) => console.error('❌ Redis error:', err.message));
  redisClient.on('connect', () => console.log('✅ Redis connected'));
  redisClient.on('ready', () => console.log('✅ Redis ready'));
  return redisClient;
};

// Vérifie l'utilisateur avec cache Redis (30 secondes)
export const verifyUserWithCache = async (id_user, pseudo, name) => {
  const redis = getRedis();
  const cacheKey = `user_verify:${id_user}:${pseudo}:${name}`;

  let cached = null;
  if (redis) {
    try { cached = await redis.get(cacheKey); } catch (e) { console.warn('[REDIS] get failed, continuing without cache', e && e.message); }
  }
  if (cached) {
    try { return JSON.parse(cached); } catch (e) { /* ignore parse error */ }
  }

  const result = await db.query(`
    SELECT u.id_user, u.pseudo, u.name, r.role_name
    FROM users u
    LEFT JOIN roles r ON u.id_role = r.id_role
    WHERE u.id_user = $1 
      AND u.pseudo = $2 
      AND u.name = $3
  `, [id_user, pseudo, name]);

  if (result.rows.length === 0) return null;
  const user = result.rows[0];

  if (redis) {
    try { await redis.setex(cacheKey, 30, JSON.stringify(user)); } catch (e) { console.warn('[REDIS] setex failed', e && e.message); }
  }

  return user;
};
